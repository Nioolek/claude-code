...
Claude Code 选择事件驱动模式，原因如下：
- 任务可能是在**隔离环境**中运行的，无法直接通知主程序
- 在工具循环的每次迭代中顺带检查，**实现简单可靠**（不需要维护定时器或长连接）
- 每次工具循环迭代之间最多 1 秒的延迟，对大多数任务来说**可以接受**
- **注意**：`pollTasks()` 函数虽然定义了（含 `POLL_INTERVAL_MS = 1000` 常量），但实际**没有被任何代码调用**——真正的检查是通过 `generateTaskAttachments()` 在工具循环和用户输入处理时触发的

### 2.4 任务状态检查机制

**重要纠正**：`pollTasks()` 函数虽然定义了（`src/utils/task/framework.ts:255`），但**从未被任何代码调用**。真正被调用的是 `generateTaskAttachments()`，它通过事件驱动的方式触发：

```
触发时机：每个工具调用回合 + 用户输入时（仅主线程）
queryLoop 工具循环 → query.ts:1580
  └→ getAttachmentMessages() → getAttachments() → 主线程时调用
       getUnifiedTaskAttachments() → generateTaskAttachments()
```

任务状态检查**不是定时轮询**，而是事件驱动的——每次工具循环迭代或用户输入时顺带检查一次：
...
`pollTasks()` 中封装的实际逻辑与 `generateTaskAttachments()` 一致（读取磁盘增量、生成偏移量补丁、通知），只是入口不同。但 `pollTasks()` 本身是死代码——如果有人调用它，它的行为是：

```
generateTaskAttachments() 执行
      │
      ├──→ 遍历所有 running 任务
      │      │
      │      ├──→ 读取磁盘输出增量
      ...
      │             └──→ 否 → 继续等待
      │
      ↓ (但注意：generateTaskAttachments 不生成通知附件——各任务类型自行处理)
```
...
> ※ 529 切换到备用模型有条件：仅当设置了 `FALLBACK_FOR_ALL_PRIMARY_MODELS` 环境变量，或者用户**不是** Claude AI 订阅者且使用的是**非自定义 Opus 模型**时才会触发模型回退。订阅用户（非 Enterprise）遇到 529 不会触发自动回退。
...
**谁可以触发回退？**
- 仅当设置了 `FALLBACK_FOR_ALL_PRIMARY_MODELS` 环境变量（覆盖所有场景），或用户**不是** Claude AI 订阅者且使用的是非自定义 Opus 模型时
- 这意味着大多数订阅用户（使用 Sonnet 等非 Opus 模型）遇到 529 时**不会**触发模型回退，而是继续重试
...
完成后，协调器直接向用户报告结果（报告是协作流程的固有部分，并非独立阶段）。
...
│  ├── pdf_password_protected PDF 有密码保护           │
│  ├── image_too_large       图片文件过大             │
│  ├── tool_use_mismatch     工具使用不匹配           │
│  ├── auth_error            认证失败                 │
...
| npm 缓存 | `@anthropic-ai/claude-` 包的老版本 | 每包保留最新 5 个 |
...
| 模型回退 | 连续 3 次 529 错误（且满足 gating 条件） | 切换到备用模型，持续使用直到手动切换 |
...
| `src/utils/task/framework.ts` | 任务注册、通知驱逐、事件驱动检查 | ~400 |
...
| 文件 | 内容 | 行数（估计） |
|------|------|-------------|
| `src/coordinator/coordinatorMode.ts` | 多 agent 编排（四阶段方法） | ~400 |
